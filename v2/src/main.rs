...
async fn main() -> Result<()> {
    ...
    for y in 0..=(((verts_per_block * blocked.height() as f64) / inc_height_unit).ceil() as usize) {
        for x in 0..=(verts_per_block * blocked.width() as f64 + (y % 2) as f64).ceil() as usize {
            let is_atomic = false;
    ...
    // while pos.1 < max_coords.1 {
    //     while pos.0 < max_coords.0 {
    //         // all are non-atomic for now, add logic for this later
    //         let is_atomic = false;
    //         let atomic = MaybeAtomicPoint::new(is_atomic);
    //         // TODO: calculate pos with multiplication to avoid accumulating error
    //         unsafe { atomic.store(is_atomic, pos) }
    //         verts.push(atomic);
    //         pos.0 += offset_width_per_vert;
    //     }
    //     pos.0 = if flipflop {
    //         0.0
    //     } else {
    //         -offset_width_per_vert
    //     };
    //     flipflop = !flipflop;
    //     pos.1 += offset_height_per_vert;
    // }
